...
const Contact = () => {
  return (
    <section className="section p-12 " id="contact">
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch py-12">
            <div className="mb-12 lg:flex-col lg:mb-0 lg:flex raleway">
                <h2 className="headline-2 lg:max-w-[24ch]  "> Contact Me for Collaboration</h2>
                <p className="mt-3 text-zinc-400 max-w-[50ch] lg:max-w-[60ch] ">Reach out today to discuss your project needs and start collaborating on something amazing!</p>
                <div className="flex items-start gap-2 mt-12">
                    {socialLinks.map(({ href, icon }, key) => (
                        <a key={key} href={href} target="_blank" className="grid w-12 h-12 text-white transition-[background-color, color] place-items-center ring-inset ring-2 ring-zinc-50/5 hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80">{icon}</a>
                    ))}
                </div>


            </div>
                <form action="https://getform.io/f/adrrpvqa" className=" xl:pl-10 2xl:pl-30" method="post">
                    <div className=" md:grid md:items-center md:grid-cols-2 md:gap-2">
                        
                        <div className="mb-4 ">
                            <label htmlFor="name" className="label ">Name</label>
                            <input type="text" name="name" id="name" placeholder="Enter your name" required autoComplete="name" className=" text-field raleway" />
                        </div>

                        <div className="mb-4">
                            <label htmlFor="email" className="label">Email</label>
                            <input type="email" name="email" id="email" placeholder="Enter your Email" required autoComplete="email" className="text-field raleway" />
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <textarea name="message" id="message" placeholder="Hi!" required className="resize-y  raleway text-field min-h-32 max-h-80"></textarea>
                        </div>
                    <button type="submit" className=" btn btn-primary [&]:max-w-full w-full justify-center raleway">Submit</button>
                </form>
        </div>
    </section>
  )
